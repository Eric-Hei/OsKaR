...
import { BRAINSTORM_ACCENT } from './brainstormLogic';
...
/**
 * Barre supérieure du Brainstorming : compteur de séance, minuteur partagé,
 * export du résumé et réinitialisation (animateur).
 */
export const BrainstormToolbar: React.FC<BrainstormToolbarProps> = ({
  chrono, remainingSec, isFacilitator, notesCount, revealedCount,
  ...
  return (
    <div className="flex flex-wrap items-center gap-3 border-b border-line bg-white px-6 py-3">
      <p className="text-sm text-muted" aria-live="polite">
        <strong className="text-navy">{revealedCount}</strong> / {notesCount} idée{notesCount > 1 ? 's' : ''} révélée{revealedCount > 1 ? 's' : ''}
      </p>
        ...
        >
          <div className="w-full max-w-sm rounded-2xl bg-white p-6 shadow-card-hover">
            ...
            <p className="mt-2 text-sm text-muted">
              Toutes les idées de la séance seront effacées pour tout le monde.
            </p>
