...
/**
 * Barre supérieure de « Disons-nous les choses » : compteurs de cartes
 * publiées, export du résumé et réinitialisation (animateur).
 */
export const DisonsToolbar: React.FC<DisonsToolbarProps> = ({
  freinCount, moteurCount, isFacilitator, onExport, onReset,
}) => {
  ...
  return (
    <div className="flex flex-wrap items-center gap-3 border-b border-line bg-white px-6 py-3">
      <p className="text-sm text-muted" aria-live="polite">
        <strong className="text-navy">{freinCount}</strong> frein{freinCount > 1 ? 's' : ''}
        {' · '}<strong className="text-navy">{moteurCount}</strong> moteur{moteurCount > 1 ? 's' : ''} publiés
      </p>
        ...
        >
          <div className="w-full max-w-sm rounded-2xl bg-white p-6 shadow-card-hover">
            ...
            <p className="mt-2 text-sm text-muted">
              Toutes les cartes et les votes de la séance seront effacés pour tout le monde.
            </p>
